...
format: 
  docx: default
  html: default
>>>>>>> 609dd528771723a2697439e62c428035a4b7ad3f
editor: source
author:
...
Many accessibility measures derive from one proposed first proposed by [@hansen1959] from definitions of demographic potential interaction defined by [@stewartDemographicGravitationEvidence1948]. This accessibility measure $A__i$ is represented in (@eq-hansen-access).

$$
A_i = \sum_{j=1}^JO_j \cdot f(c_{ij})
$$ {#eq-hansen-access}

Where: the accessibility score $A$ at spatial unit $i$ is a weighted sum of the number of opportunities $O$ at $j$. The weights in this summation are a function of the cost of travel, $f(c_{ij})$, usually called a distance-decay or impedance function. $f(c_{ij})$ reflects how the potential for interaction changes with the cost of travel $c_{ij$ between spatial units $i$ and $j$, that is the origin and destination of a potential trip. Generally, the more costly it is to reach a destination, the lower the potential for interaction.

The cost $c_{ij$ in the distance-decay function can be distance, time, cost, or a combination of several factors, which is why more _impedance_ is a somewhat more general term than distance-decay, since the function models the impedance of travel. Generally $f(c_{ij})$ decays with growing cost (the impedance is greater), and so oppotunities $O_j$ at destinations that are less costly to reach are more heavily weighted in the summation. Conversely, opportunities that are costly to reach (i.e., they are *far* away in cost space) have values of  $f(c_{ij})$ close to or equal to zero, so a negligible amount of $O_j$ enter the summation.

In short: the impedance function $f(c_{ij})$ allows the accessibility analyst to precisely define a measure of travel behavior: the relationship between where people are (population) and where they usually go, want to go, or can go to reach opportunities.
...
- [Binary (or uniform) function]()
- [Other functions and insert hyperlinks]
-

The binary function is shown in (@eq-binary-access), and is the foundation of the cumulative opportunities measure approach (this measure is discussed in the next post **LINK TO POST2**). The binary function is so called because it returns only two value, which for simplicity are often 1 or 0. 1 is assigned if the travel cost from $i$ to $j$ is below some threshold $T$, that is the opportunity is reachable within this cost. Conversely, it is 0 if the travel cost is above a certain threshold $T$, meaning the opportunity exceeds the cost that people are willing to pay to reach it. Threshold $T$ should be selected carefully to reflect the observed or assumed travel behavior for the situation of interest.
...
Next, three more complex impedance $f(c_{ij})$ functions are shown: these functions are [_probability density functions_](https://en.wikipedia.org/wiki/Probability_density_function) (PDF). In conceptualizing $f(c_{ij})$ as forms of PDF, the $f(c_ij)$ values (the probability density value along the y-axis) can be interpreted as the _probability_ of a trip occurring for each value of $c_ij$. The probability of a trip between a certain range of $c_ij$ is the area under the curve (the area under a PDF always equals to 1, i.e., 100% probability that the trip between the minimum and maximum $c_ij$ will occur). Let's go over three examples of commonly used theoretical PDFs in accessibility impedance functions.

The uniform function looks very similar to the binary (shown in (@eq-binary-access)), as it only returns one of two values. However, it also has the property that those values ensure that the area under the curve for the range of $c_ij$ is 1 (or 100%). The theoretical _uniform density function_ is shown in (@eq-uniform-imped). The parameters which the analyst chooses are $T_{max}$ and $T_{min}$: they equal the maximum and minimum travel costs (i.e., the range) that describe the observed or assumed willingness or ability to reach destinations. Outside of this range, the potential for interaction is zero. 
...
The next two functions are the exponential density function and the gamma density function (part of the exponential function facilities but utilizes the gamma function $\Gamma(\alpha)$). These two functions are shown in (@eq-exp-imped) and (@eq-gamma-imped). The analyst selects parameters for these functions represented by $\lambda$ (exponential) and $\alpha$ and $\sigma$ (gamma) <!-- What are these parameters?? -->

. These two functions can take a value of _relative liklihood_ from 0 to approaching infinity for all positive $c_{ij}$, where the range depends on the analyst-defined parameters.
...
In all three PDF, the analyst must define the parameters. A useful technique is to calibrate the parameters using empirically observed origin-to-destination (OD) travel data. This empirical data can be used to build a trip length distribution (TLD) <!-- what is this? --> and then to estimate parameters for the selected theoretical PDF can be identified. 

I will demonstrate this process as follows on a sample of empirical home-to-work flows from Hamilton Center from the R data package [{TTS2016R}](hyperlink) [@TTS2016r]. The flows are taken at the spatial unit of traffic analysis zones (TAZ). This package contains a subset of home-to-work flows the 2016 Transportation Tomorrow Survey (TTS) as well as calculated road-network car travel times (calculated using [{r5r}]() [@r5r_2021]). {TTS2016R} is detailed in this publication [@soukhovTTS2016RDataSet2023] and is freely available [here](https://soukhova.github.io/TTS2016R/). The TLD for this empirical data is shown in black in the plot below:
...
The TLD is the _empirical_ PDF of the travel costs associated with the OD trips. Like all PDFs, the y axis represents the probability of the value at the x axis of occurring. In our context, $f(c_{ij})$ is the probability of a trip at a given travel time (in minutes, $c_{ij}$). It can be observed that the probability of travelling is highest when travel time is around `r empirical |> filter(f == max(f)) |> pull(x) |> prettyNum(digits=2)` minutes (`r max(empirical$f) |> scales::percent()` in occurring). It can also be seen that around the 45 min mark, the probability levels off. In other words, the probability of a trip of length 0 to 45 mins occurring is  `r sum(diff(sort(red_empirical$x)) * 0.5 * (red_empirical$f[-1] + head(red_empirical$f, -1))) |> scales::percent()`. Trips outside of this range thus make up the remaining probability. 
...
-   Uniform function ($f(c_{ij})^*$ - red): $T_{min}$ and $T_{max}$ is 0 and `r unif_$estimate[2] |> prettyNum(digits=1)` mins, respectively.
...
Functions in @fig-TLD-all can be interpreted in the following way: the higher the $f(c_{ij})$, the higher the probability of travelling to reach the opportunities at the destination.^[Using {fitdistrplus}, the parameters in all theoretical functions were selected through an optimization algorithm that minimizes the differences between all possible parameter range(s) and the empirical function for each theoretical function.]
